...
class GeoModel(ndb.Model):
  ...
  Attributes:
    location: A ndb.GeoPt that defines the single geographic point
  ...
  def bounding_box_fetch(query, bbox, max_results=1000,
    ...
    Args:
      query: A ndb.Query on entities of this kind that should be additionally
    ...
    Raises:
      Any exceptions that google.appengine.ext.ndb.Query.fetch() can raise.
    """
  ...
  def proximity_fetch(query, center, max_results=10, max_distance=0):
    ...
    Args:
      ...
      center: A geotypes.Point or ndb.GeoPt indicating the center point around
