angular.module('xplore.api').factory('loginApi', [ '$http', function($http){
  var api = {};
  ...
  return api;
} ]).factory('missionsApi', [ '$http', function($http){
  var api = {};
  api.missions = function(){
    var promise = $http({
      url : '/api/missions',
      params : {
        detailed : false
  ...
  api.detailedMission = function(missionName){
    var promise = $http({
      url : '/api/missions/' + encodeURIComponent(missionName),
      method : 'GET'
    }).then(function(httpData){
      if (httpData.status == 200) {
        return httpData.data;
